...
from src.models.models import db, Role, AreaOfResponsibility, Skill, License, EmployeeLicense, User, Shift, ShiftRoster, Timesheet
...
def init_database():
    ...
    with app.app_context():
        ...
        licenses_data = [
            ...
            {'name': 'Food Safety', 'description': 'Food handling and safety certification'},
            {'name': 'Leadership', 'description': 'Leadership/Management certification'}
        ]
        ...
        first_aid = License.query.filter_by(name='First Aid').first()
        forklift = License.query.filter_by(name='Forklift').first()
        food_safety = License.query.filter_by(name='Food Safety').first()
        ...
        # Assign licenses with expiry dates
        from datetime import date, timedelta
        if jane and leadership_skill and first_aid:
            db.session.add(EmployeeLicense(employee_id=jane.id, license_id=first_aid.id, expiry_date=date.today() + timedelta(days=20)))
        if bob and food_safety:
            db.session.add(EmployeeLicense(employee_id=bob.id, license_id=food_safety.id, expiry_date=date.today() - timedelta(days=5)))
        if alice and forklift:
            db.session.add(EmployeeLicense(employee_id=alice.id, license_id=forklift.id, expiry_date=date.today() + timedelta(days=400)))
